...
#              - him_read_message()
#              - parse_output()
#  - TODO: read_message() ; compose_message() ; change_account() ; 
...
# DEBUG $viewport_ratio
# echo "DEBUG: _viewport_ratio var = $viewport_ratio"
...
# DEBUG $_orientation
#echo "DEBUG: _orientation var = $_orientation"
...
him_message_preview="$him_path message read {1} --html --header="From,To,Cc,Subject" \
...
chosen=$(printf '%s\n' "Menu" "$search_query" | SHELL=/bin/bash FZF_DEFAULT_COMMAND="$search_query" fzf \
  --disabled \
  --ansi \
  --separator="_" \
  --query "$search_query" \
  ...
  --bind "double-click:replace-query+refresh-preview" \
  --bind "enter:show-preview" \
  --bind "enter+print($him_message_preview)+refresh-preview" \
  --bind ">:reload:$search_query" \
  --bind 'down:refresh-preview+down' \
  --bind 'up:refresh-preview+up' \
  --bind 'esc:close' \
  --print-query
)
...
#### USE LATER (for clickable header) ###
# 
# [...]
# --bind 'click-header:transform-prompt:printf ${FZF_CLICK_HEADER_LINE}x${FZF_CLICK_HEADER_COLUMN}'
#
...
# DEBUG point for OUTPUT PARSING
...
# Menu option was chosen
  Menu) 
#    echo "DEBUG: Setting search_query to ${menulist}"
    search_query="$menulist"
#    echo "DEBUG: Setting fzf_prev_list to ${default_search_query}"
    fzf_prev_list="$default_search_query"
    ;;

# Change Account option was chosen
  "Change Account")
#    echo "DEBUG: Setting search_query to ${accounts_fzf_input}"
    search_query="$accounts_fzf_input"
#    echo "DEBUG: Setting fzf_prev_list to ${menulist}"
...
# Back option was chosen
  "Back")
#    echo "DEBUG: Setting search_query to ${fzf_prev_list}"
    search_query="$fzf_prev_list"
#    echo "DEBUG: Setting fzf_prev_list to null"
    fzf_prev_list=""
    ;;
...
#    echo "DEBUG: Running menu again with new vars"
  menu ; parse_output
fi
...
# Set up viewport / UI and himalaya data variables for first run, built menus
get_viewport_dims
him_get_accounts
him_get_envelopes
build_menus
...
### DEBUG SWITCHES for first menu run ###
